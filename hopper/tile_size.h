...
constexpr std::tuple<int, int, bool, bool> tile_size_fwd_sm90(
    ...
    if (element_size == 2) {
        if (headdim <= 64) {
            ...
            // Forced kBlockN=128 (was: 128 if causal/local/paged_kv_non_TMA else 176).
            // Smaller kBlockN gives finer per-CTA work granularity, which helps
            // sparse / decode workloads where Sq=1 leaves the M dim under-filled.
            // May regress dense long-Sq prefill throughput.
